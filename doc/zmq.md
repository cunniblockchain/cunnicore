...
The Cunni Core daemon can be configured to act as a trusted "border
router", implementing the cunni wire protocol and relay, making
...
The ZeroMQ feature in Cunni Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building cunnid:
...
For instance:
    ...
    $ cunnid -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/cunnid.tx.raw \
...
These options can also be provided in cunni.conf.
...
From the perspective of cunnid, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into cunnid directly. Furthermore, no information is
...
using. Cunnid appends an up-counting sequence number to each
