...
const char * const BITCOIN_CONF_FILENAME = "cunni.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //cunni-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Cunni
    // macOS: ~/Library/Application Support/Cunni
    // Unix-like: ~/.cunni
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cunni";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Cunni";
#else
    // Unix-like
    return pathRet / ".cunni";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty cunni.conf if it does not exist
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    std::string strCopyrightHolders = strPrefix + strprintf(" %u ", nEndYear) + copyright_devs;
