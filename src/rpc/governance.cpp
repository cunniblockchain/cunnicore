...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 CUN irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on cunnicentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Cunni features */
    { "cunni",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "cunni",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "cunni",               "gobject",                &gobject,                {} },
    { "cunni",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
