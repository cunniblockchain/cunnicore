...
impl PrivateKey {
    ...
    pub fn generate_cunni(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "cunni_helpers")]
    pub fn generate_cunni_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_cunni(rng))
            .collect()
    }
